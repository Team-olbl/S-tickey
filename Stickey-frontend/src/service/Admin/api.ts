...
export const patchOrganizationStateReq = async ({id, status, message} :{id : number, status: string, message: string}): Promise<APIResponse<string>> => {
...
export const patchPostStateReq = async ({id, status, message} :{id : number, status: string, message: string}): Promise<APIResponse<string>> => {
