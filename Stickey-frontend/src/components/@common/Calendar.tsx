    ...
    <div className="flex flex-row text-white px-4 py-4">
      ...
      <div ref={containerRef} className="flex flex-row gap-5 overflow-auto pl-4">
        {calendarData.map((item, id) => (
          <div key={id} ref={todayDate == item.day ? dateRef : undefined} className={`flex flex-col  items-center px-3 ${selectedDate === item.day ? 'rounded-2xl bg-gray-600 text-white' : ''}`} onClick={() => handleClick(item.day)} data-day={item.day}>
          <button>
