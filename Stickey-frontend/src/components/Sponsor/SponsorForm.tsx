  ...
  return (
    <>
      <div className="px-4 py-16">
        ...
        <div>
          ...
          <div className="w-full flex gap-3">
            <input
              ...
              onChange={e =>
                setSupportReq(state => ({ ...state!, startTime: dayjs(e.target.value).format('YYYY-MM-DDTHH:mm:ss') }))
              }
            />
            <input
              type="date"
              ...
              onChange={e =>
                setSupportReq(state => ({ ...state!, endTime: dayjs(e.target.value).format('YYYY-MM-DDT23:59:59') }))
              }
            />
          </div>
        </div>
        ...
        <div
          className="fixed bottom-16 w-full max-w-[320px] m-auto"
          style={{ left: '50%', transform: 'translate(-50%,0)' }}
        >
          <button
            className={`${isEnable ? `bg-[#5959E7]` : `bg-Stickey_Gray`} w-full text-white rounded-md p-2 text-sm`}
            onClick={handleSupportRegist}
            disabled={!isEnable}
          >
            승인 요청하기
          </button>
