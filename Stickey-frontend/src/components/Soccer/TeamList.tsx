    ...
    <div className="px-4 flex flex-row z-[2] overflow-x-auto">
      <div className="flex flex-row gap-2">
        {teamListInfo && teamListInfo?.data.map((team) => (
          <div key={team.id} onClick={() => handleTeamClick(team.name)} className={`relative w-14 h-16 border border-none flex flex-col shadow-[2px_2px_rgba(0,0,0,0.25)] justify-center items-center gap-1 rounded-[5px] ${selectedTeams.includes(team.name) ? 'bg-black/50' : 'bg-[#2E2E3D]'} `}>
            {team.isPrefer === 1 && (
              <img src={Star} className='absolute w-3 h-3 left-1 top-1'/>
            )}
