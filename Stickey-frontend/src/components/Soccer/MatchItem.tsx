...
const MatchItem = ({ data }: { data: IGameSimpleRes }) => {
  ...
  return (
    <>
          ...
          <div className="flex flex-col items-center pt-4">
            <button
              className={`w-full h-[36px] border-none rounded-[10px] flex justify-center items-center ${
                bookingStatus === 'notopen' || bookingStatus === 'close'
                  ? 'bg-gray-400'
                  : bookingStatus === 'ticketing'
                    ? 'bg-Stickey_Main cursor-pointer'
                    : ''
              }`}
              onClick={bookingStatus === 'ticketing' ? handleBookTicket : undefined}
            >
              {bookingStatus === 'notopen' && <p className="text-sm text-center">{bookStart} 오픈예정</p>}
              {bookingStatus === 'ticketing' && <p className="text-sm text-center">티켓 예매하기</p>}
              {bookingStatus === 'close' && <p className="text-sm text-center">마감 되었습니다</p>}
            </button>
          </div>
        </div>
      </div>
      ...
      {/* 모달 */}
      {isModalOpen && (
        <BottomModal height="auto" title="경기장 내 안전 수칙" onClose={() => setIsModalOpen(false)}>
            ...
            <div className="flex flex-col space-y-2">
              ...
              <div className="flex items-center p-2 bg-[#E4F7FE] mb-1">
                ...
                <p className="font-bold text-sm px-2">
                  정치적, 종교적 문구 / 비방 문구가 포함된 게시물 반입 및 게시 금지
                </p>
              </div>
            </div>
