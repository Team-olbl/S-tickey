...
const TicketOpenModal: React.FC<TicketOpenModalProps> = ({ ticket, onClose, getData }) => {
    ...
    return (
        ...
        <div className='fixed top-0 w-full h-screen max-w-[500px] bottom-0 bg-black/70 z-[2]'>
                ...
                {/* modal wrapper */}
                <div className="flex justify-center items-center rounded-lg pt-[20vh]"  ref={scope}>
                            ...
                            <div ref={containerRef} className="text-center">
                                ...
                                <div>
                                    ...
                                    <div className='flex justify-center max-w-[300px] h-auto'>
                                        ...
                                        <img onClick={handleEditClick} className="fixed right-24 mt-6 bg-white/70 w-8 h-8 rounded-full p-1" src={Edit} alt="Edit" />
                                    </div>
                                    :
                                    <div>
                                    ...
                                    <div className={`background${ticket.backgroundId} rounded-t-lg text-center p-2`}>
                                        <p className='text-sm p-1'>{dayjs(Number(ticket.gameStartTime) * 1000).format('YY/MM/DD HH:mm')} {ticket.stadium}</p>
                                        <p className="font-bold text-2xl">{ticket.zoneName} {ticket.seatNumber}번 좌석</p>
                                        <p>결제금액 : {toEther(ticket.price)} ETH</p>
                                        <div className="px-4 py-3 flex justify-center">
                                            <button className="z-[1] bg-gray-400 mx-1 p-2 rounded-md text-white"  onClick={handleQR}>QR 전환</button>
                                            <button onClick={handleRefund} className="z-[1] bg-gray-400 mx-1 p-2 rounded-md text-white">환불하기</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
