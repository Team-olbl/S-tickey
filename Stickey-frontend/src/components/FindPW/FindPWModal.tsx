...
const FindPWModal = ({ onClose }: { onClose: () => void; }) => {
  ...
  //     },
  //     onError: (error) => {
  //         toast.error(`오류가 났습니다. ${error.message}`)
  //     }
  //   });
  //   console.log(email)
  // }
    ...
    <Modal width="340px" height="auto" title="비밀번호 찾기" onClose={onClose}>
      <div className="">
        ...
        <div className="flex items-center justify-center px-4">
          ...
          />
          <button className="w-20 h-7 text-sm border border-Stickey_Main text-Stickey_Main rounded-[14px]">인증하기</button>
        </div>
      </div>
      <div className="flex items-center justify-center px-12 py-6" onClick={handleSendPassword}>
        <button className="border border-none bg-Stickey_Main text-white rounded-full w-full h-8" >임시 비밀번호 발급 받기</button>
      </div>
    </Modal>
  )
}
