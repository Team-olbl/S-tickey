  ...
  return (
    <Modal width="330px" height="auto" title="비밀번호 변경" onClose={onClose}>
      <div className="">
        <div className="flex flex-col items-center justify-center px-4 pt-10 pb-4">
          ...
          {isPasswordValid === false && (
            <p className="pl-14 text-xs text-red-500">비밀번호 형식이 유효하지 않습니다.</p>
          )}
        </div>
        <div className="flex flex-col items-center px-4">
          ...
          {isPasswordMatch === false && (
            <p className="pl-14 text-xs text-red-500">비밀번호가 서로 일치하지 않습니다.</p>
          )}
        </div>
      </div>
      {}
      <div className="flex items-center justify-center px-20 py-6">
        <button
          className={`border border-none bg-Stickey_Main text-white rounded-full w-full h-8 ${isButtonEnabled ? 'bg-Stickey_Main' : 'bg-gray-500'}`}
