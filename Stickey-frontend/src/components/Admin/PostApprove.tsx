...
const PostApprove = () => {
  ...
  return (
    <>
      <div>
        <div className="p-4">
          {supportPosts && supportPosts.waitingSupportRes.length > 0 ? (
              ...
              <div className="m-2 p-4 bg-[#2E2E3D] rounded-md text-white" key={post.id}>
                ...
                <div className="flex items-center justify-between">
                  <p className="py-1 text-xs">
                    {post.organization.name} ( userID {post.organization.id} )
                  </p>
                  <button
        ...
        {isModalOpen && <PostApproveDetail id={selectId} onClose={handleModalClose} refetch={refetch} />}
      </div>
    </>
  );
};
