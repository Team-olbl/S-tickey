  ...
  return (
    <>
      <div className="max-w-[500px] w-full h-[208px] mt-4 border-t-[0.5px]">
        <div className="flex flex-row items-center justify-between h-[40px] text-white px-4 cursor-pointer" onClick={() => { if (status) { navigate('/profile/playerlist') } else toast.warn("승인을 받아야합니다.")}}>
          <p>선수 조회 / 수정</p>
          <img src={Next} className="h-[20px]"/>
        </div>
        <div className="flex flex-row items-center justify-between h-[40px] text-white px-4 cursor-pointer" onClick={() => { if (status) { navigate('/profile/sponlist') } else toast.warn("승인을 받아야합니다.")}}>
          <p>후원글 내역</p>
          <img src={Next} className="h-[20px]"/>
        </div>
