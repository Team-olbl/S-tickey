            ...
            <div onClick={() => navigate(`/sponsor/${data.id}`)}>
              <div className="h-auto flex border-b-[0.5px] border-Stickey_Gray p-4 gap-3">
                ...
                <div className="text-white grow px-2">
                  <div>
                    <div
                      className={`w-10 h-4 flex justify-center items-center rounded-full ${data.status === 'ACCEPTED' ? `${dayjs(data.endTime) > dayjs() ? `bg-green-500` : `bg-blue-500`}` : data.status === 'REJECTED' ? 'bg-red-500' : 'bg-Stickey_Gray'} `}
                    >
                      <p className="text-[9px]">
                        {data.status === 'ACCEPTED'
                          ? `${dayjs(data.endTime) > dayjs() ? `진행 중` : `마감`}`
